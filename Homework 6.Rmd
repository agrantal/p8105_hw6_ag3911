...
p2_tidy = read_csv("./birthweight.csv") %>%
  janitor::clean_names() %>%
  select(fincome, bwt, frace, mrace, blength, gaweeks, bhead, babysex) %>%
...
mod_1_plot = p2_tidy %>%
  ...
  add_residuals(mod_1) %>%
  ggplot(aes(x = fincome, y = bwt)) + geom_point() + geom_line(aes(y = pred), color = "red") + geom_line(aes(y = resid), color = "blue")
mod_1_plot
