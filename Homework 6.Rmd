...
all_city = tidy %>%
  group_by(city_state) %>%
  nest() %>% 
  mutate(output = map(data, ~glm(resolved ~ age + race2 + sex, data = .x, family = binomial()))) %>%
  mutate(output = map(output, broom::tidy)) %>%
  select(-data) %>%
  unnest()
```
...
all_city %>% 
  ...
  filter(term == "race2nonwhite") %>%   
  mutate(conf_high = exp(log_OR + (1.96*std.error))) %>%
  mutate(conf_low = exp(log_OR - (1.96*std.error))) %>%
  group_by(city_state) %>%
  ggplot(aes(x = reorder(city_state, -OR), y = OR)) + geom_errorbar(aes(ymin = conf_low, ymax = conf_high)) + geom_point() + labs(x = "Location", y = "Odds Ratio", title = "Homicides Solved for Non-White versus White Victims by City") + theme(axis.text.x = element_text(angle = 90))
```
In Tampa, Florida and Birmingham, Alabama being non-white has a slightly positive impact on the likelihood that a homicide will be solved. In most cities and in Boston, Massachussetts, in particular, being non-white has the greatest negative impact on the likelihood that a homicide will be solved. Also, the confidence intervals are much wider near an OR = 1 than near an OR = 0.  
