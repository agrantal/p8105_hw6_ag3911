...
tidy = read_csv("./homicide-data.csv") %>%
  ...
  mutate(city_state = str_c(city, ", ", state),  #new var combines city and state
         ...
         victim_race = recode(victim_race,
                          `White` = "white",
                          `Black` = "nonwhite",
                          `Hispanic` = "nonwhite",
                          `Other` = "nonwhite",
         ...
         #victim_race = fct_relevel(victim_race, c("white", "nonwhite")),
...
#mutate(victim_race = as.numeric(victim_race == "non-white")) #so white is 0, non-white is 1
...
Plot of model residuals against fitted values using  add_predictions and add_residuals. 
```{r}
